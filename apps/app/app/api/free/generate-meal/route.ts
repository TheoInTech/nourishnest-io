...
export async function POST(req: NextRequest) {
  ...
  const REMEMBER_CONTEXT = `Remember, strictly follow each fields especially age, dietary restrictions and allergies! We don't want to suggest meals that are not suited with the age, diet and allergies. For example, babies that are just starting to eat are recommended to have the same meal for 3 days for familiarity. Take note of those special cases for all ages. Strictly generate 1 week worth (7 days) of complete meal plan including ${frequencyOfMeals} as mandatory, per week should be 7 days, start on week 1.\n
  \n--
